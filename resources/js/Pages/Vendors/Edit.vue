    ...
    <AuthenticatedLayout>
        <div class="page-wrap">
            ...
            <div class="page-header">
                <div>
                    <div class="breadcrumb">
                        ...
                        <Link :href="route('vendors.show', vendor.id)" class="bc-link">{{ vendor.business_name }}</Link>
                        <span class="bc-sep">›</span>
                        <span class="bc-cur">Edit</span>
                    </div>
                    <div class="eyebrow-row">
                        <span class="eyebrow-dot" style="background:#F05A00"></span>Edit
                        <span v-if="form.isDirty" class="dirty-badge">
                            <span class="dirty-pulse"></span>Unsaved changes
                        </span>
                    </div>
                    <h1 class="page-title">Edit: {{ vendor.business_name }}</h1>
                    <p class="page-sub">Update vendor details below</p>
                </div>
            </div>
            ...
            <div v-if="showSummary && Object.keys(clientErrors).length > 0" class="val-summary">
                ...
                <div>
                    <p class="val-title">Please fix the following errors before saving:</p>
                    <ul class="val-list">
            ...
            <div class="form-layout">
                        ...
                        <div class="ep-card form-card">
                            <div class="card-head">
                                ...
                                <div>
                                    ...
                                    <div class="card-sub">Admin-only verification controls</div>
                                </div>
                            </div>
                            <div class="card-body">
                                <div class="field-grid">
                                    <div>
                                        ...
                                        <select v-model="form.verification_status" class="ep-input ep-select ep-input-plain">
                                    ...
                                    <div>
                                        <label class="field-label">Verification Notes <span class="opt">(optional)</span></label>
                                        <input v-model="form.verification_notes" type="text" placeholder="Optional notes for the vendor…" class="ep-input ep-input-plain">
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="form-sidebar">
                    ...
                    <!-- Vendor Summary -->
                    <div class="ep-card">
                        <div class="card-head" :class="form.isDirty ? 'head-dirty' : ''">
                            <span class="card-icon" style="background:rgba(240,90,0,.1)">📋</span>
                            <div>
                                <div class="card-title">{{ form.isDirty ? 'Unsaved Changes' : 'Vendor Summary' }}</div>
                                <div class="card-sub" :style="form.isDirty ? 'color:#b45309' : ''">{{ form.isDirty ? 'Review before saving' : 'Live preview of vendor' }}</div>
                            </div>
                        </div>
                        <div class="dl-list">
                            <div class="dl-row">
                            ...
                            <div class="dl-row">
                                <span class="dl-label">Verification</span>
                                <span class="status-chip" :style="VERIF_STYLE[form.verification_status] || VERIF_STYLE.default">
                                    <span class="status-dot" :style="{background: VERIF_DOT[form.verification_status] || '#9E9890'}"></span>
                                    {{ form.verification_status ? form.verification_status.charAt(0).toUpperCase() + form.verification_status.slice(1) : '—' }}
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="sidebar-actions">
                        ...
                        <button v-if="form.isDirty" type="button" @click="resetForm" class="btn-amber btn-full">
                        ...
                        <Link :href="route('vendors.show', vendor.id)" class="btn-ghost btn-full">
                            <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><path d="M19 12H5M12 19l-7-7 7-7"/></svg>
                            Discard &amp; View
                        </Link>
                    </div>
...
.eyebrow-row{display:flex;align-items:center;gap:7px;font-family:'DM Mono',monospace;font-size:10px;letter-spacing:.18em;color:#9E9890;text-transform:uppercase;margin-bottom:5px;flex-wrap:wrap}
.eyebrow-dot{width:6px;height:6px;border-radius:50%;animation:blink .9s ease-in-out infinite;flex-shrink:0}
@keyframes blink{0%,100%{opacity:1}50%{opacity:.2}}
.dirty-badge{display:inline-flex;align-items:center;gap:5px;padding:3px 9px;border-radius:20px;background:rgba(249,178,51,.12);color:#b45309;border:1px solid rgba(249,178,51,.4);font-family:'DM Mono',monospace;font-size:9px;font-weight:700;text-transform:none;letter-spacing:0}
.dirty-pulse{width:5px;height:5px;border-radius:50%;background:#b45309;animation:pulse 1s ease-in-out infinite}
...
.head-dirty{background:rgba(249,178,51,.12)!important;border-bottom-color:rgba(249,178,51,.3)!important}
...
.ep-input-plain:focus{border-color:#1D5C96!important;box-shadow:0 0 0 3px rgba(29,92,150,.09)!important}
...
.tag-rm{background:none;border:none;cursor:pointer;font-size:9px;opacity:.6;padding:0;line-height:1;transition:opacity .15s}.tag-rm:hover{opacity:1}
...
.thumb-on{left:20px}.thumb-off{left:2px}
...
.btn-ghost{display:inline-flex;align-items:center;justify-content:center;gap:6px;padding:10px 18px;border-radius:12px;border:1.5px solid #E8E2DA;background:#fff;color:#6B6560;font-size:12px;font-weight:600;font-family:'DM Sans',sans-serif;text-decoration:none;cursor:pointer;transition:all .18s}
.btn-ghost:hover{border-color:#9E9890;color:#1A1410}
.btn-amber{display:inline-flex;align-items:center;justify-content:center;gap:6px;padding:10px 18px;border-radius:12px;border:1.5px solid rgba(249,178,51,.4);background:rgba(249,178,51,.1);color:#b45309;font-size:12px;font-weight:700;font-family:'DM Sans',sans-serif;cursor:pointer;transition:all .18s}
.btn-amber:hover{background:rgba(249,178,51,.18)}
